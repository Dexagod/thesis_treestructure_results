...
datasets = ["stops", "realistic"]

dictionary = dict({"seriesindex": [], "execution time": [], "results": [], "fragment size": [], "datastructure": [], "dataset": []})
for dataset in datasets:
  ...
  for datastructure in datastructures:
    for fragmentsize in fragment_sizes:
      ...
      querytimings = [list() for _ in range(5)]
...
ax = sns.relplot( x = "execution time" , y="results", hue="fragment size", hue_order = fragment_sizes, col="seriesindex", row="dataset", data=dataframe, linewidth=2.5, kind="line", palette=sns.color_palette("Set1", dataframe["fragment size"].nunique()) )
...
for d in [r'437 $\times 10^3$ triples', r'124 $\times 10^6$ triples']:
  # for s in ["street names", "transport stops", "OSMnames"]:
  for s in ["query 1", "query 2", "query 3", "query 4", "query 5"]:
    axes[i].set_title( d + " | " + s)
    i += 1

plt.subplots_adjust(top=0.9)
plt.gcf().suptitle('Average query results in 150ms over subsequent queries in a series')
plt.savefig(file_path("results/performance/"+ "+".join(datasets) +'.png'))
